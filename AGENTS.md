...
- Local directories to inspect first when relevant: `sdkwork-ui-mobile-flutter/`, `sdkwork-ui-mobile-react/`, `sdkwork-ui-pc-react/`.
...
Build-critical source files under `sdkwork-ui-pc-react/build/` (e.g., `package-contract.ts`, `docs-governance-contract.ts`, `generated-reference-contract.json`) are git-tracked source contracts, not build artifacts. They must not be deleted by `clean` or cache purges. Build runners must verify and self-heal these files per `CODE_STYLE_SPEC.md` §7.
